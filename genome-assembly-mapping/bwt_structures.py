...
def forward_bwt(seq):
    ...
    for i in range(len(seq)):
        BW_matrix.append(seq[i:len(seq)] + seq[:i])
    BW_matrix.sort()
    result_seq = '' 
    for item in BW_matrix:
        result_seq += item[-1:] 
    return result_seq
...
def reverse_bwt(seq):
    ...
    given_mat = []
    for i in range(len(seq)):
        given_mat.append(seq[i])
    result_mat = [i for i in given_mat]
#     j = 0
#     while j < len(seq)-1:
    for j in range(len(seq)-1):
        result_mat.sort()
        for k in range(len(result_mat)):
            result_mat[k] = given_mat[k] + result_mat[k]
#         j += 1
    result_mat.sort()
    result_seq = result_mat[0][1:]+result_mat[0][0]
    return result_seq
...
if __name__ == '__main__':
    ...
    seq2 = 'ACTGA$TA'
    seq4 = 'TTAGAC$AG'
    print 'The BWT result of the given sequence is:', forward_bwt(seq1)
    print 'The reverse BWT result of the given sequence is:', reverse_bwt(seq2)
    print 'The BWT result of the problem 3a sequence is:', forward_bwt(seq3)
    print 'The reverse BWT result of the problem 3b sequence is:', reverse_bwt(seq4)
